...
function Grammar(Token, All, Any, Plus, Optional, Node) {
  ...
  return Y(function (Expression) {
    ...
    Token(
      /(=>|\.\.\.|\|\||&&|>>>|>>|<<|<=|>=|\btypeof\b|\binstanceof\b|\bin\b|===|!==|!=|==|\+\+|--|\bnew\b|[{}[\]().?:|&=,^%*/<>+\-~!])/g,
      'verbatim',
    );
    ...
    const Identifier = Node(IdentifierToken, ([name], $) => ({
      type: 'Identifier',
      name,
      anchors: [$.context.tokens[$.ti].start, $.context.tokens[$.ti].end],
    }));
    ...
    const RegExLiteral = Node(RegExToken, ([raw, flags]) => ({
      type: 'Literal',
      value: new RegExp(raw, flags),
      raw: `/${raw}/${flags || ''}`,
    }));
    ...
    const SpreadElement = Node(All('...', Expression), ([expression]) => ({
      type: 'SpreadElement',
      expression,
    }));
    ...
    const ObjectLiteral = Node(All('{', PropertyDefinitionList, '}'), (properties) => ({
      type: 'ObjectLiteral',
      properties,
    }));

    // Primary expression
    const PrimaryExpression = Any(
      Identifier,
      Literal,
      ArrayLiteral,
      ObjectLiteral,
    ...
    const MemberExpression = Node(
      All(PrimaryExpression, Star(Any(PropertyAccess, Arguments))),
      (parts) =>
    ...
    const NewExpression = Node(All('new', MemberExpression), ([expression]) => ({
      type: 'NewExpression',
      expression,
    }));
    ...
    const UnaryExpression = Node(All(Star(UnaryOperator), LeftHandSideExpression), (parts) =>
      parts.reduceRight((argument, operator) => ({ type: 'UnaryExpression', argument, operator })),
    );
    ...
    const ApplyBinaryOp = (BinaryOp, Expr) =>
      Node(All(Operator(BinaryOp), Expr), ([operator, right]) => ({ operator, right }));
    const ExpressionConstructor = (Expr, BinaryOp) =>
      Node(All(Expr, Star(ApplyBinaryOp(BinaryOp, Expr))), (parts) =>
        parts.reduce((left, { operator, right }) => ({
    ...
    const LogicalORExpression = BinaryOperatorPrecedence.reduce(
      ExpressionConstructor,
      UnaryExpression,
    );

    const ConditionalExpression = Node(
      All(LogicalORExpression, Optional(All('?', Expression, ':', Expression))),
    ...
    const BindingElement = Node(
      All(IdentifierToken, Optional(All('=', Expression))), // Do not support destructuring just yet
      ([name, initializer]) => (initializer ? { name, initializer } : { name }),
    );
    const FormalsList = Node(All(BindingElement, Star(All(',', BindingElement))), (bound) => ({
      bound,
    }));
    const RestElement = Node(All('...', IdentifierToken), ([rest]) => ({ rest }));

    const FormalParameters = Node(
      All('(', Any(All(FormalsList, Optional(All(',', RestElement))), Optional(RestElement)), ')'),
      (parts) => parts.reduce((acc, part) => Object.assign(acc, part), { bound: [] }),
    );

    const ArrowParameters = Node(Any(IdentifierToken, FormalParameters), ([params]) =>
      params.bound ? params : { bound: [{ name: params }] },
    );
    ...
    const ArrowFunction = Node(All(ArrowParameters, '=>', ArrowResult), ([parameters, result]) => ({
      type: 'ArrowFunction',
      parameters,
      result,
    }));
